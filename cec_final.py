...
def draw(total_rewards, name, config):
...
# train function
# config object
# amount_of_training_to_do, eval_loops, gamma,
# close_neighbour_threshold_action_selection, close_neighbour_threshold_memory
# noise_probability, noise_std_dev
# softmax_temperature
    ...
    for i in range(config["amount_of_training_to_do"]):
        ...
        while len(episode_mem) > 0:
            ...
            if (
                ...
                or the_closest_state_neighbour[0]
                > config["close_neighbour_threshold_memory"]
            ):
...
def main():
    for env in games:
        for i, params in enumerate(hyperparameter_sets):
            ...
            draw(total_rewards, f"{env}_{i}_training", params)
