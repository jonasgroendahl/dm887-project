...
class TreeMemory:
    def __init__(self, autoencoder, max_memory_size=1000, leaf_size=5):
    ...
    def add(self, s_a_v_tuple):
        ...
        encoded_state = self.encode_states(self.autoencoder, [s_a_v_tuple[0]])[0]
    ...
    def lookup(self, state_vector, k=5):
        ...
        latent_state = self.encode_states(self.autoencoder, [state_vector])[0]
    ...
    def update(self, index, state_reward_action_pair):
        encoded_state = self.encode_states(
            self.autoencoder, [state_reward_action_pair[0]]
        )[0]
...
def train_autoencoder(data, input_dim, latent_dim, num_epochs=20, batch_size=32):
...
games = ["Hopper-v4", "HalfCheetah-v4", "Ant-v4"]
...
def main():
    for env in games:
        for i, params in enumerate(hyperparameter_sets):
            ...
            num_episodes_for_autoencoder = 20000
            autoencoder_data = []
            ...
            autoencoder = train_autoencoder(
                autoencoder_data, state_size, params["latent_dim"]
            )
            print("Training autoencoder finished")

            mem = TreeMemory(
                max_memory_size=params["max_memory_size"], autoencoder=autoencoder
            )
            ...
            draw(total_rewards, f"{env}_{i}_training", params, use_mean=True)
            draw(total_rewards_eval, f"{env}_{i}_evaluation", params)
